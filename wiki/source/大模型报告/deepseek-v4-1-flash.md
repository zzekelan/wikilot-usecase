...
总训练 **45T token**；固定 batch **100.6M token**；从 64K 稀疏注意力起训、无 dense warmup，在 34T 扩至 1M。前 2000 步线性预热，至 28T 保持 2.6×10⁻⁴，28T–40T 余弦降到 2.6×10⁻⁵，40T–45T 保持；28T 开始解冻视觉编码器（第 15、22 页）。详见[[wiki/synthesis/优化与训练/学习率与批量的联合调度|训练调度]]。
...
多模态路由 bias 更新速率均 .001，同时仍保留权重 .0001 的序列级平衡损失；“auxiliary-loss-free”不能理解为整个训练完全无平衡辅助项。使用 sample-level attention mask（第 22 页）。基础解释见[[wiki/concept/优化与调度/Adam|优化器]]、[[wiki/synthesis/学习与表示/数据质量与泛化评估|数据与泛化]]。
...
BPB 与 PPL 同源于负对数概率但归一化不同；例如单 token 概率 .5 对应单步 PPL=2，**不是 BPB=2**。完整定义见[[wiki/synthesis/学习与表示/交叉熵-PPL与BPB的换算|概率预测指标换算]]。作者说 API 无法进行此类测试，应读作其所用 API 的限制；支持充分 logprob 的其他接口未必如此。更低 BPB 不等于已验证能独立完成研发。

独立指标页：[[wiki/concept/概率与采样/困惑度PPL|困惑度（PPL）]]解释 token 归一化，[[wiki/concept/概率与采样/每字节比特数BPB|BPB]]解释字节归一化；采样协议中的 [[wiki/concept/概率与采样/核采样top-p|top-p]] 则控制候选截断，不是概率评测指标。

## 五、后训练与智能体环境

作者明确沿用 **[[wiki/concept/后训练与智能体/监督微调SFT|SFT]]→[[wiki/concept/后训练与智能体/强化学习RL|RL]]→[[wiki/concept/后训练与智能体/在策略蒸馏OPD|OPD]]**，不主张新后训练算法，主要归因于数据/环境工程；其“几乎全部提升来自数据”的归因属于作者结论，报告未提供足够隔离消融让本 wiki 独立确认（第 6、25 页）。
...
协议（第 32、35、47–48 页）：推理温度/top-p=1/1；代码智能体通常 DSH Minimal、1M context、1/.95，DeepSWE 改用 mini-SWE，SEC-Bench Pro（260505）用 Claude Code；视觉 agent 用 Claude Code、512K；AutomationBench v1.0.6 公开集与 ALE-CLI 用官方 scaffold。评测限制网络、移除 Git 历史和临时 build/package cache，仍观察到投机行为。不能把工具辅助结果当成纯模型闭卷分数。采样概念见[[wiki/synthesis/学习与表示/从logits到生成采样|从 logits 到生成采样]]。
...
与[[wiki/source/深度学习基础/深度学习基础-2026暑期学校|基础课件]]的数学联系见[[wiki/synthesis/总览与练习/深度学习基础与DeepSeek报告的共同概念|共同概念对照]]；与[[wiki/source/模型评测/chatbot-arena|Arena 论文]]共同关注评测的人群、任务、采样与统计不确定性。

## 关联条目（持续维护）

- [[wiki/synthesis/优化与训练/矩阵参数与嵌入表的优化器选择|Muon 与 Sinkhorn 矩阵优化]]：展开混合优化器方案及算法 1。

- [[wiki/synthesis/架构与系统/预填充-解码与缓存复用|KV 缓存与预填充、解码]]：解释报告降低部署成本时使用的缓存与推理阶段口径。

- [[wiki/concept/架构与注意力/CED因果编码器解码器|CED 因果编码器–解码器]]：展开降低 prefill 计算的因果编码器–解码器。

- [[wiki/concept/架构与注意力/CSA2压缩稀疏注意力|CSA2 压缩稀疏注意力]]：解释架构图中的 Full、Reindex 与 Reuse。

- [[wiki/concept/架构与注意力/分层稀疏索引|分层稀疏索引]]：展开分层候选池的选择与复杂度。

- [[wiki/concept/推理与缓存/SWA有界重放|SWA 有界重放]]：展开运行时与持久缓存之间的存储–重算权衡。

- [[wiki/concept/推理与缓存/FP4缓存量化|FP4 缓存量化]]：说明主 KV 低精度布局与 QAT。

- [[wiki/synthesis/架构与系统/稀疏专家与模态负载控制|MoE 与多模态负载均衡]]：展开 DeepSeekMoE 的专家配置与图文分模态路由。

- [[wiki/concept/架构与注意力/Single-Pass-mHC|Single-Pass mHC]]：解释 Mega-mHC 内核融合所依赖的结构改变。

- [[wiki/entity/模型与组件/Engram|Engram]]：记录条件记忆模块的配置与训练/部署关系。

- [[wiki/concept/推理与缓存/推测解码|推测解码与 DSpark]]：解释 DSpark 的草拟、验证与训练阶段。

- [[wiki/entity/模型与组件/DeepSeek-ViT|DeepSeek-ViT]]：集中说明视觉编码器及其两阶段训练。

- [[wiki/entity/模型与组件/DeepSeek-V4.1-Flash|DeepSeek-V4.1-Flash]]：作为模型实体总览，连接架构组件与完整结果。

- [[wiki/synthesis/架构与系统/分布式执行的解耦-共享与融合|分布式训练与推理解耦]]：串联多模态训练、共享状态、Engram 分片和 EPD 部署。

- [[wiki/synthesis/后训练与智能体/SFT-RL与OPD的分工|SFT、RL 与 OPD 的分工]]：整理后训练流程与多教师蒸馏。

- [[wiki/synthesis/后训练与智能体/智能体任务的构建与质量闭环|智能体任务合成与验证]]：展开自动任务生产、质检和修复循环。

- [[wiki/entity/智能体系统/DSec|DSec]]：记录智能体沙箱平台的身份、调度和隔离设计。

- [[wiki/synthesis/后训练与智能体/异步采样的分布与状态管理|异步 RL 与离策略样本]]：解释异步后训练调度和两类偏差修正。

- [[wiki/concept/后训练与智能体/推理力度|推理力度与测试时计算]]：展开力度条件、长度奖励、预算曲线和附录推导。

- [[wiki/synthesis/后训练与智能体/模型-框架与评测协议的比较边界|智能体框架与评测协议]]：解释跨框架成绩差异与复现配置。

- [[wiki/entity/智能体系统/DeepSeek-Harness|DeepSeek Harness]]：集中记录框架模式与协作接口。

- [[wiki/synthesis/后训练与智能体/多智能体协作的时间与计算成本|多智能体协作与关键路径]]：解释多智能体协作奖励与初步实验。

- [[wiki/synthesis/偏好与评测/评测指标的对象与可比条件|评测指标与可比性]]：统一解释结果表中的不同指标与采样口径。

- [[wiki/synthesis/偏好与评测/评测泄漏-选择偏差与奖励投机的治理|评测污染与奖励投机]]：串联任务质检、沙箱隔离和评测防投机。

- [[wiki/synthesis/总览与练习/从学习目标到智能体与人类偏好评测|从学习目标到智能体与人类偏好评测]]：从基础学习目标连接到智能体系统及多层评测。
