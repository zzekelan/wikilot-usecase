...
- 原题：*DeepSeek-V4.1-Flash: Pushing the Limits of KV Cache Compression*；署名：DeepSeek-AI。研究工程、商务与合规作者名单见附录 A，第 46–47 页；不据排列次序推断个人贡献。
- 原文：[[raw/大模型技术报告/DeepSeek-V4.1-Flash/DeepSeek_V41_Tech_Report.pdf|本地 PDF]]，51 页。报告链接：[模型仓库](https://huggingface.co/deepseek-ai/DeepSeek-V4.1-Flash)、[报告地址](https://huggingface.co/deepseek-ai/DeepSeek-V4.1-Flash/blob/main/DeepSeek_V41_Tech_Report.pdf)。本次未重新联网核验版本或权重。
- 年份：2026；第 30 页称生产部署于 2026 年 9 月上线，具体报告发布日期不明。PDF 创建时间 2026-09-10 不是发布日期证明。
- 2026-09-16 已阅读全文 1–51 页，包括参考文献与附录；视觉核对第 25、33、48、49 页。下文均用 **PDF 页码**，不依赖可能偏移的书签目标。未复现实验，也未独立审计内部语料、模型或部署指标。
- [[raw/大模型技术报告/DeepSeek-V4.1-Flash/img_v3_0215e_1d574a93-1181-47ad-af6e-e46320b7f04g.png|后训练截图]]已核对为第 **6 页**“Building on this base model…”段落，不是第 25 页；它是同一来源片段，不算独立证据。

## 核心论点与章节地图

**作者论点：** 长时程智能体具有输入重、前缀复用多、上下文长的负载特征，瓶颈不只在注意力 FLOPs，也在 KV 的 HBM/SSD 容量和搬运。报告协同改变架构、缓存精度、状态恢复与训练数据，主张在能力提升时继续降成本。**Wiki 判断：** 这是模型与服务系统的联合方案，不能用单一参数量或某个榜单分数概括。出处：第 1、4–6、37 页。
...
| 38–45 | 参考文献；仅为报告引用线索，不代表本 wiki 已读其全文 |
...
**表 3（第 33 页，已视觉核对）是作者报告，不是独立榜单认证。** 以下列顺序与原表一致；K3 为正文所称 Kimi-K3；横线不表示零。HLE 的 † 是 text-only 子集，不能与完整 HLE 混比。除 Codeforces rating 外下列为百分制/报告评分。
...
- 本次取代此前“局部精读”“图 6 待核验”“截图页码未知”状态；保留早期 R&D 问答的定义与证据边界，图 6 数值现已补全。内部语料规模与隔离审计仍未知。

与[[wiki/source/深度学习基础-2026暑期学校|基础课件]]的数学联系见[[wiki/synthesis/深度学习基础与DeepSeek报告的共同概念|共同概念对照]]；与[[wiki/source/chatbot-arena|Arena 论文]]共同提示：评测必须明确人群、任务、采样与统计不确定性，不能把作者成绩表当完整能力结论。
