...
CED（Causal Encoder–Decoder）把因果 Transformer 的前半层视为 encoder，后半层视为 decoder。与经典双向编码器的序列到序列模型不同，这里的 encoder 也是因果的，整个模型仍自回归生成文本。目标是降低输入占比很高的智能体工作流中的[[wiki/concept/推理与缓存/预填充|prefill 成本]]。来源：[[wiki/source/大模型报告/deepseek-v4-1-flash|报告]]第 7–9 页。
...
- [[wiki/concept/架构与注意力/CSA2压缩稀疏注意力|CSA2 压缩稀疏注意力]]：在 CED 的 KV 生成机制上叠加跨层共享。

- [[wiki/concept/推理与缓存/SWA有界重放|SWA 有界重放]]：提供 decoder 局部 KV 的低成本恢复路径。
